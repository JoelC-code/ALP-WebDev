<div>
    <div class="custom-field-item d-flex justify-content-between align-items-center p-2 border mb-2">
        @if ($editMode)
            <div class="w-100">
                <input type="text" wire:model="fieldTitle" class="form-control form-control-sm mb-2">
                ...
                @if ($editMode && $field->exists && $fieldType !== $field->getOriginal('type'))
                    <div class="alert alert-warning alert-sm py-1 px-2 mb-2">
                        <small>⚠️ Changing type will remove all existing values from cards</small>
                    </div>
                @endif
                ...
                @if ($fieldType === 'select')
                    <div class="mb-2">
                            ...
                            @foreach ($customOptions as $index => $option)
                                <div class="d-flex align-items-center gap-2 mb-1">
                                    @if ($editingOptionIndex === $index)
                                        <input type="text" wire:model="editingOptionLabel"
                                            class="form-control form-control-sm flex-grow-1"
                                            wire:keydown.enter="saveEditOption" wire:keydown.escape="cancelEditOption">
                                    ...
                                    @else
                                        <span class="badge bg-secondary flex-grow-1 text-start cursor-pointer"
                                            wire:click="startEditOption({{ $index }})" title="Click to edit">
                                            {{ $option['label'] }}
                                        </span>
                        ...
                        <div class="input-group input-group-sm">
                            <input type="text" wire:model="newOptionLabel" class="form-control"
                                placeholder="Add new option..." wire:keydown.enter="addOption">
                            <button class="btn btn-outline-primary" type="button" wire:click="addOption">
                                Add
                            </button>
                        </div>
                    </div>
